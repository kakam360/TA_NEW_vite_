...
---------------------------------------------------*/
	...
	window.ScrollEffects = function() {
		...
		let enableSmoothScrollMobile = false; // disabled on mobile to reduce scroll jank
		if( isMobile() ){
		...
		function calculatePixels() {
			...
			if (firstWrapper) {
				...
				if (parentRow) {
					...
					bgColor = document.getElementById('TravellersArchive-page-content').getAttribute('data-bgcolor');
				}
			} else {
				bgColor = document.getElementById('TravellersArchive-page-content').getAttribute('data-bgcolor');
			}
		...
		if (isMobile()) {
			...
			function setViewPortHeight() {
				...
				gsap.set(".smooth-scroll main, .has-parallax, .TravellersArchive-slider-wrapper:not(.content-slider), .showcase-lists .TravellersArchive-sync-slider, .slider-fixed-content", { height: winHeight});
			}
		...
		if ($('body').hasClass("swap-logo")) {
            ...
            // No logo images are used in this project; keep original sources empty
			...
			ScrollTrigger.create({
				trigger: '.TravellersArchive-header', // Set the trigger element
				start: 'top 120px',
				onEnter: function() {
				...
				onEnterBack: function() {
					...
					$('.TravellersArchive-header').removeClass('swapped-logo');
				},
				onLeave: function() {
					...
					$('.TravellersArchive-header').addClass('swapped-logo');			
				},
		...
		if ($("#hero-interaction").length) {
			...
			$(interactionTarget).mouseenter(function(e) {   
				...
				$("#ball").append('<img class="eyes-icon" src="src/images/eyes.png" style="transform:scale(0.75);">');                
			
				let imageIndex = 0;
				const images = [
					'<img class="eyes-icon" src="src/images/eyes1.png" style="transform:scale(0.75);">',
					'<img class="eyes-icon" src="src/images/eyes.png" style="transform:scale(0.75);">' 
				];
		...
		gsap.utils.toArray('.clipped-image-wrapper').forEach((clippedImageWrapper) => {
			...
			function setClippedImageWrapperProperties() {
				...
				gsap.set(clippedImage, { height: window.innerHeight - document.querySelector('.TravellersArchive-header').offsetHeight * 2 });								
				gsap.set(clippedImageContent, { paddingTop: (window.innerHeight/2) + clippedImageContent.offsetHeight});
				gsap.set(clippedImageWrapper, { height: window.innerHeight + clippedImageContent.offsetHeight - document.querySelector('.TravellersArchive-header').offsetHeight});
		...
		function setMainPageContentMargins() {
			var winHeight = $(window).height();
			var footerHeight = $('.TravellersArchive-footer').height();
		...
		gsap.to('.page-nav-caption', {
			...
			scrollTrigger: {
				trigger: '#page-nav',
				start: startValue,
				end: () => `+=${$('#page-nav').outerHeight() + $('.TravellersArchive-footer').outerHeight()}`,
				scrub: true,
			}			
		});
			...
			imagesLoaded('body', function() {
				setTimeout(function() {
					...
					changeHeaderColor.forEach(function(changeHeaderElement) {						
						var pageHeader = document.querySelector('.TravellersArchive-header');							
			...
			if ($('#project-nav').hasClass("change-header")) {
		  		imagesLoaded('body', function() {
					setTimeout(function() {	
						...
						if (pageNav) {
							var pageContent = document.querySelector('#TravellersArchive-page-content');	
		...
		if ($('#TravellersArchive-page-content').hasClass("dark-content")) {
			if ($('#project-nav').hasClass("change-header")) {
		  		imagesLoaded('body', function() {
					setTimeout(function() {	
						var pageNav = document.querySelector('#project-nav.change-header');
		...
		gsap.set('.header-gradient', {
			delay: 0.5,
			backgroundColor: document.getElementById('TravellersArchive-page-content').getAttribute('data-bgcolor'),			
			onUpdate: function() {
			var bgColor = document.getElementById('TravellersArchive-page-content').getAttribute('data-bgcolor');
		...
		$('a.ajax-link').on('click', function() {
			...
			gsap.to('#TravellersArchive-logo img.white-logo, #TravellersArchive-logo img.black-logo', { duration: 0.5, delay:0, opacity: "",  ease: Linear.easeNone, });
...
---------------------------------------------------*/	
	...
	window.FirstLoad = function() {
		...
		if ($("#TravellersArchive-page-content").hasClass("light-content")) {
			$(".TravellersArchive-nav-wrapper").css('background-color', function () {
				return $(".TravellersArchive-header").data('menucolor')
			});
			
			gsap.to("main", {duration: 0.5, backgroundColor: document.querySelector("#TravellersArchive-page-content").getAttribute("data-bgcolor"), ease:Power2.easeInOut});
			
			$('#magic-cursor').addClass('light-content');
			if( $('#hero').length > 0 ){						
				if( $('#hero').hasClass("has-image")) {	
					$(".TravellersArchive-header").css('background-color', 'transparent');
				} else {
					if ($(".TravellersArchive-header").hasClass("fullscreen-menu")) {
						$(".TravellersArchive-header").css('background-color', 'transparent');
					} else {
						if( $('#blog').length > 0 ){
							$(".TravellersArchive-header").css('background-color', '#171717');
						}
						if( $('#post').length > 0 ){
							$(".TravellersArchive-header").css('background-color', '#171717');
						}
			...
			if( $('#hero').length > 0 ){	
				if( $('#hero').hasClass("has-image")) {	
					...
					if ($(".TravellersArchive-header").hasClass("fullscreen-menu")) {
						...
						if( $('#blog').length > 0 ){
							$(".TravellersArchive-header").css('background-color', '#fff');
						}
						if( $('#post').length > 0 ){
							$(".TravellersArchive-header").css('background-color', '#fff');
						}
		...
		$('a.ajax-link').on('click', function() {
			...
			gsap.to("nav", {duration: 0.3, backgroundColor: document.querySelector("#TravellersArchive-page-content").getAttribute("data-bgcolor")});
		...
		$('.TravellersArchive-nav-wrapper .ajax-link').on('click', function() {
		...
		$('#burger-wrapper, .menu .button-text').on('click', function() {
			$('#menu-burger, .TravellersArchive-nav-wrapper').toggleClass('open');			
			setTimeout( function(){			
				if ($('#menu-burger').hasClass("open")) {
					
					gsap.to('.TravellersArchive-nav-wrapper', {duration: 0.3, opacity:1, ease:Power2.easeInOut});
					
					$('.TravellersArchive-header').addClass('over-sidebar').addClass('over-white-section');
					if (!$('#page-content').hasClass("light-content")) {	
						$('#magic-cursor').addClass('light-content');
					}
					if ($('.TravellersArchive-header').hasClass("invert-header")) {
						$('#header-container').addClass('light-content-header');
					} else {
						$('#header-container').addClass('dark-content-header');
					}
					gsap.set($(".TravellersArchive-nav-wrapper ul ul li"), {y: 0, opacity:1});
					...
					gsap.to('.TravellersArchive-nav-wrapper', {duration: 0.3, opacity:0, delay:0.6, ease:Power2.easeInOut});
						
					//Fade Out Navigation Lists						
					gsap.to($(".menu-timeline .before-span"), {duration: 0.5, y:-200, opacity:1, delay:0, stagger:0.05, ease:Power2.easeIn});
					gsap.to($(".TravellersArchive-nav-wrapper ul ul li"), {duration: 0.5, y:-120, opacity:0, delay:0, stagger:0.03, ease:Power2.easeIn});
					...
					setTimeout( function(){
						...
						$('.TravellersArchive-header').removeClass('over-sidebar')
						setTimeout( function(){
							$('.TravellersArchive-header').removeClass('over-white-section');
						} , 350 );
					} , 500 );
				}							
			} , 20 );
				  ...
				  $body.on('mouseup mousemove', function handler(evt) {
					if (evt.type === 'mouseup') {
					  ...
					  $('#itemsWrapperLinks .trigger-item-link, #itemsWrapperLinks .trigger-item-link-secondary').on('click', function() {					
							...
							if (!$('#TravellersArchive-page-content').hasClass("light-content")) {
								...
								if (!$('.portfolio').hasClass("portfolio-shortcode")) {
									if (!parent_item.hasClass("change-header")) {
										$('#TravellersArchive-page-content').delay(700).queue(function(next){							
							...
							$('.TravellersArchive-slider-wrapper .trigger-item').each(function(){
							...
							gsap.to('#hero, #show-filters, .item-caption-wrapper, .showcase-portfolio .slide-caption, #page-nav, .TravellersArchive-footer, .fadeout-element', {duration: 0.5, opacity:0, ease:Power4.easeInOut});
...
---------------------------------------------------*/
	...
	window.Sliders = function() { 
		...
		if( $('.content-slider').length > 0 ){
			...
			slider = new TravellersArchiveSlider('.content-slider', { 
			...
			$(".slider-button-prev").mouseenter(function(e) {				
				var modifyMouseColor = $('#TravellersArchive-page-content').attr("data-modify-color");
				if (modifyMouseColor) {
					...
					if ($(this).parents(".TravellersArchive-slider-wrapper").hasClass("light-cursor")) {
						$("body").addClass("drag-cursor-white");
						gsap.to('#ball', {duration: 0.2, borderWidth: '2px', scale: 1, borderColor:'#fff', backgroundColor:'#fff'});
					} else if ($(this).parents(".TravellersArchive-slider-wrapper").hasClass("dark-cursor")) {
			...
			$('.content-slider.looped-carousel .TravellersArchive-slider, .content-slider.small-looped-carousel .TravellersArchive-slider').on('mouseenter mousemove', function() {	
...
---------------------------------------------------*/
	...
	window.Lightbox = function() {
		...
		function showDetails(item) {
			...
			$('body').prepend(`<div class="Travellers' Archive-img-popup">
				<div class="Travellers' Archive-img-popup-bg-close"></div>				
				<div class="Travellers' Archive-img-popup-viewport">
					<div class="TravellersArchive-img-popup-preloader"><div></div><div></div><div></div><div></div></div>					
					<img />
					<div class="Travellers' Archive-img-popup-close link"></div>						
				</div>				
				<div class="Travellers' Archive-img-popup-prev link"></div>
				<div class="Travellers' Archive-img-popup-next link"></div></div>`);
			
			const details = document.querySelector('.TravellersArchive-img-popup');
			const detailsBgClose = document.querySelector('.TravellersArchive-img-popup-bg-close');
			const detailsClose = document.querySelector('.TravellersArchive-img-popup-close');
			const detailsPreloader = document.querySelector('.TravellersArchive-img-popup-preloader');
			const detailImage = document.querySelector('.TravellersArchive-img-popup img');
			const detailPrev = document.querySelector('.TravellersArchive-img-popup-prev');
			const detailNext = document.querySelector('.TravellersArchive-img-popup-next');
		...
		function hideDetails() {
			...
			Flip.from(state, {
				...
				onComplete: () => gsap.to(detailsBgClose, {duration: 0.5, backgroundColor:"rgba(0,0,0,0)", onComplete: function() {
					$('.TravellersArchive-img-popup').remove();
				}}),				
			});
		...
		function showVideoDetails(event, item) {
			...
			$('body').prepend(`<div class="TravellersArchive-video-popup">
				<div class="TravellersArchive-video-popup-bg-close"></div>				
				<div class="TravellersArchive-video-popup-viewport">
					<div class="TravellersArchive-video-popup-preloader"><div></div><div></div><div></div><div></div></div>					
					<iframe class="TravellersArchive-video-popup-iframe" frameborder="0" allow="autoplay"></iframe>
					<div class="TravellersArchive-video-popup-close link"></div>						
				</div>				
				<div class="Travellers' Archive-img-video-prev link"></div>
				<div class="Travellers' Archive-img-video-next link"></div></div>`);
			
			const details = document.querySelector('.TravellersArchive-video-popup');
			const detailsBgClose = document.querySelector('.TravellersArchive-video-popup-bg-close');
			const detailsClose = document.querySelector('.TravellersArchive-video-popup-close');
			const detailsPreloader = document.querySelector('.TravellersArchive-video-popup-preloader');
			const detailIframe = document.querySelector('.TravellersArchive-video-popup iframe');
		...
		function hideVideoDetails() {
			...
			gsap.to(detailsBgClose, {duration: 0.5, delay:0.2, backgroundColor:"rgba(0,0,0,0)", onComplete: function() {
					$('.TravellersArchive-video-popup').remove();
				}}),
...
---------------------------------------------------*/
	...
	window.Core = function() {
		...
		if (!isMobile() && !$('body').hasClass("disable-cursor")) {
			...
			$(".TravellersArchive-button .sticky.left").mouseenter(function(e) {		  
				var rcBounds = $(this)[0].getBoundingClientRect();		  
			...
			$(".TravellersArchive-button .sticky.right").mouseenter(function(e) {		  
				var rcBounds = $(this)[0].getBoundingClientRect();		  
			...
			$(".TravellersArchive-button .parallax-wrap.icon-wrap").mouseenter(function(e) {
			...
			$(".TravellersArchive-button .sticky").mouseenter(function(e) {
				if ($('#TravellersArchive-page-content').hasClass("light-content")) {
			...
			$(".TravellersArchive-button .parallax-wrap").mouseenter(function(e) {
				if ($('#TravellersArchive-page-content').hasClass("light-content")) {
			...
			//Blog Hover Effects			
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .TravellersArchive-sidebar-widget a").mouseenter(function(e) {	
				gsap.to('#ball', {duration: 0.2, borderWidth: '1px', scale: 1, opacity:0});
			});	
					
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .TravellersArchive-sidebar-widget a").mouseleave(function(e) {
				gsap.to('#ball', {duration: 0.3, borderWidth: '4px', scale:0.5, opacity:1});
			});
		}
			  ...
			  $(window).on('popstate', function() {
				...
				section.load(url+' .cd-main-content > *', function(event){
				  // load new content and replace <main> content with the new one
				 	...
				 	var TravellersArchive_title = event.match(/<title[^>]*>([^<]+)<\/title>/)[1];
					$('head title').html( TravellersArchive_title );
