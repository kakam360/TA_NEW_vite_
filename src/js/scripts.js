...
---------------------------------------------------*/
	...
	function CustomFunction() {
        ...
        // Custom code for Traveler's Archive
        // Make the hero title stacked on small screens when at the top of the page.
        function adjustHeroTitle() {
            var heroTitle = document.querySelector('#hero .hero-title');
            if (!heroTitle) return;
            // Get the inner span that holds the text
            ...
            // Determine if we should stack: small screen and near top
            var isMobile = window.innerWidth <= 768;
            var atTop = window.pageYOffset < 50;
            if (isMobile && atTop) {
                heroTitle.classList.add('stacked');
                // Insert a line break between words to stack them
                var original = span.dataset.original;
                span.innerHTML = original.replace(/\s+/, '<br>');
            } else {
                heroTitle.classList.remove('stacked');
        ...
        // Attach listeners to adjust on load, scroll, and resize
        window.addEventListener('load', adjustHeroTitle);
        window.addEventListener('scroll', adjustHeroTitle);
...
---------------------------------------------------*/			
	...
	function InitShuffleElements() {	
		...
		shuffleFunctions.applyShuffleEffect(".showcase-portfolio .TravellersArchive-item .slide-inner", {
			velocity: 30,
			shuffleIterations: 8,
			childSelector: '.slide-title span, .slide-cat span'
		});
		
		shuffleFunctions.applyShuffleEffect(".showcase-gallery .TravellersArchive-slide", {
			velocity: 30,
			shuffleIterations: 8,
			childSelector: '.slide-title span, .slide-cat span'
		});
...
---------------------------------------------------*/	
	...
	function CleanupBeforeAjax(){		
		...
		TravellersArchiveSlider.instances.forEach(slider => slider.off());
		TravellersArchiveSlider.instances = [];
	}
...
---------------------------------------------------*/
	...
	function PageLoad() {		
		...
		gsap.set($(".TravellersArchive-header"), {yPercent:-50, opacity:0});
		...
		function initOnFirstLoad() {
			...
			imagesLoaded('body', function() {
				...
				gsap.to($(".TravellersArchive-header"), { duration: 0.45, opacity:1, yPercent:0, delay:0.25, ease: Power2.easeOut, });
	...
	function PageLoadActions() {
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			$('#page-nav .next-ajax-link-page').on('click', function() {	
				...
				$('.TravellersArchive-header').removeClass('white-header');
				$("#app").remove();
				...
				gsap.to($("#main-page-content, #hero, .TravellersArchive-footer, .TravellersArchive-footer-nav"), {duration: 0.3, opacity:0, ease:Power4.easeOut});
				
				if ($("#page-nav").hasClass("move-nav-onload")) {
					$("body").addClass("load-next-page");
					if ($("body").hasClass("smooth-scroll")) {
						var moveNav = $("#content-scroll").height() - ( $("#hero").height() / 2 )  - ( $("#page-nav").height()  ) / 2 - $(".TravellersArchive-footer").height() / 2	
					} else {
						var moveNav = window.innerHeight - ( $("#hero").height() / 2 )  - ( $("#page-nav").height()  ) / 2 - $(".TravellersArchive-footer").height() / 2	   
					}
			...
			$('#page-nav .next-ajax-link-page').on('click', function() {					
				...
				gsap.to($(".TravellersArchive-footer, .TravellersArchive-footer-nav"), {duration: 0.3, opacity:0, delay:0, ease:Power2.easeInOut});
			});
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			$('.next-ajax-link-project').on('click', function() {
				...
				if ($(".TravellersArchive-header").hasClass("swapped-logo")) {
					var imgLogoWhite = document.querySelector('.white-logo');
                    // No logo images are used; set original sources to empty strings
                    var originalSrcWhite = '';
					var updatedSrcWhite = 'src/images/logo-white-symbol.png';
					
					var imgLogoBlack = document.querySelector('.black-logo');
                    var originalSrcBlack = '';
					var updatedSrcBlack = 'src/images/logo-symbol.png';
					gsap.to($("#TravellersArchive-logo"), {duration: 0.2, opacity:0, onComplete: function() {
						imgLogoWhite.src = originalSrcWhite;
						imgLogoBlack.src = originalSrcBlack;
						gsap.to($("#TravellersArchive-logo"), {duration: 0.2, opacity:1});
					}});
				...
				gsap.to($(".TravellersArchive-footer, .all-works"), {duration: 0.3, opacity:0, ease:Power2.easeInOut});
		...
		if( $('#project-nav').length > 0 ){		
			...
			const pageContent = document.getElementById("TravellersArchive-page-content");
			...
			ScrollTrigger.create({
				...
				onLeaveBack: function() {
					...
					gsap.set($("#TravellersArchive-logo img, .classic-menu .flexnav li, .button-wrap.menu, .button-icon-link"), { clearProps: "all" });
				},			
				scrub: true,
			});	
			...
			if ($("#project-nav").hasClass("change-header")) {
				...
				if (changeMainColor) {
					...
					imagesLoaded('body', function() {
						...
						const blackLogoOpacity = gsap.getProperty('#TravellersArchive-logo img.black-logo', 'opacity');
						const whiteLogoOpacity = gsap.getProperty('#TravellersArchive-logo img.white-logo', 'opacity');
						...
						ScrollTrigger.create({
							...
							onEnter: function( st ) {
								...
								gsap.fromTo('#TravellersArchive-logo img.black-logo', {
								...
								gsap.fromTo('#TravellersArchive-logo img.white-logo', {
									opacity: whiteLogoOpacity
								}, {
									duration: 1, 
									opacity: blackLogoOpacity,
...
---------------------------------------------------*/	
	...
	function ShowcasePortfolio() {
		...
		gsap.utils.toArray('.TravellersArchive-item .pixels-cover').forEach((pixelWrapper, index) => {
			const pixelAnimation = pixelWrapper.querySelectorAll(".pixel");
		...
		if( $('.showcase-portfolio').length > 0 ){
			...
			const pageContent = $('#TravellersArchive-page-content');
			...
			function filter() {				
				...
				var state = Flip.getState('.TravellersArchive-item');
				var projects = document.querySelectorAll('.TravellersArchive-item');
				var startHeight = gsap.getProperty(".showcase-portfolio", "height");
				
				var filters = document.querySelectorAll('.filter-option.is_active');
				var parallaxItems = document.querySelectorAll('.showcase-portfolio .TravellersArchive-item.vertical-parallax .slide-inner');
			...
			if (!isMobile()) {	
				...
				$(".showcase-portfolio .TravellersArchive-item .slide-inner").on('mouseenter', function() {
					$('#ball p').remove();
					var $this = $(this);			
...
---------------------------------------------------*/
	...
	function ShowcaseHighlights() {
		...
		if( $('.highlights-gallery').length > 0 ){
			...
			var slideElements = $(".TravellersArchive-slide");
			...
			if (!$('body').hasClass("show-loader")) {
				...
				setTimeout(function() {
					...
					let firstSlide = document.querySelector(".TravellersArchive-slide");
			...
			slider = new TravellersArchiveSlider('.highlights-gallery', {
				ease: 0.05,  
				direction: 'vertical', 
				snap: true,
				outer: '.TravellersArchive-slider',		
				inner: '.TravellersArchive-slider-viewport',	
				webgl: true,
				...
				on: {	
					init : function(slide) {
						...
						if (firstSlide.classList.contains("change-header")) {
							...
							gsap.to('#TravellersArchive-logo img.black-logo', { duration: 0.3, opacity: 1, ease:Power2.easeInOut });	
							gsap.to('#TravellersArchive-logo img.white-logo', { duration: 0.3, opacity: 0, ease:Power2.easeInOut });						
							...
							gsap.to('#TravellersArchive-logo img.black-logo', { duration: 0.3, opacity: 0, ease:Power2.easeInOut });	
							gsap.to('#TravellersArchive-logo img.white-logo', { duration: 0.3, opacity: 1, ease:Power2.easeInOut });						
			...
			const arrTitles = gsap.utils.toArray('.TravellersArchive-caption-wrapper .external-titles .slide-title');
			const arrCategories = gsap.utils.toArray('.TravellersArchive-caption-wrapper .external-categories .slide-cat');
			...
			if (!isMobile()) {
				...
				$('.TravellersArchive-slider').on('mousedown', function (evt) {
					$('.TravellersArchive-slider').on('mouseup mousemove', function handler(evt) {
						if (evt.type === 'mouseup') {					  
							// click
							gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
						...
						$('.TravellersArchive-slider').off('mouseup mousemove', handler);
					});
				});
				
					
				$('.TravellersArchive-slider').on('mouseup touchend', function() {
				...
				$(".external-titles .slide-title").on('mouseenter', function() {	
					if (!$('body').hasClass('scale-drag-y')) {
						...
						let relatedSlide = $('.TravellersArchive-slide[data-slide="' + dataCaption + '"]');
			...
			$('.external-titles .slide-title').on('click', function () {
				...
				let dataCaption = $(this).attr('data-caption'); 
				let parentSlide = $('.TravellersArchive-slide[data-slide="' + dataCaption + '"]');
				...
				gsap.to('.TravellersArchive-caption-wrapper', { duration: 0.5, opacity: 0, filter: "blur(5px)", ease: Power4.easeInOut });
...
---------------------------------------------------*/
	...
	function ShowcaseGallery() {
		...
		if( $('.showcase-gallery').length > 0 ){
			...
			gsap.set($(".showcase-gallery .TravellersArchive-slider .slide-effects"), { opacity: 0 });
			...
			if (!$('body').hasClass("show-loader")) {
				...
				setTimeout(function() {
					...
					gsap.to($(".showcase-gallery .TravellersArchive-slide .slide-effects"), { duration: 0.5, opacity: 1, delay: 1.1, ease: Power4.easeIn, onComplete: function() {
						$(".showcase-gallery").addClass("active");
					}});
					
					var gallerySlideClasses = [".TravellersArchive-slide-prev", ".TravellersArchive-slide-active", ".TravellersArchive-slide-next"];
					gallerySlideClasses.forEach(function(gallerySlideClass, index) {
						var gallerySlide = $(".showcase-gallery .TravellersArchive-slider " + gallerySlideClass + " .slide-effects");
			...
			const slidesRoot = document.querySelector('.TravellersArchive-sync-slider .TravellersArchive-sync-slider-wrapper .TravellersArchive-sync-slider-viewport');
			const slidesList = slidesRoot.querySelectorAll('.TravellersArchive-sync-slider .TravellersArchive-sync-slide');
			
			let slidesHeight = 0;
			
			for (const TravellersArchiveSyncSlide of slidesList) {
				slidesHeight += TravellersArchiveSyncSlide.offsetHeight;
			}
			...
			if (iterCloning >= 1) {
				...
				for (let i = 0; i < iterCloning; i++) {
					for (const TravellersArchiveSyncSlide of slidesList) {
						let cloneSlide = TravellersArchiveSyncSlide.cloneNode(true);
						slidesRoot.appendChild(cloneSlide);
					}
				}
			}
			
			const TravellersArchiveSyncSlider = document.querySelector('.TravellersArchive-slider-wrapper.showcase-gallery .TravellersArchive-sync-slider-viewport');
			const syncSliderClone = TravellersArchiveSyncSlider.cloneNode(true);
			document.querySelector(".TravellersArchive-slider-wrapper.showcase-gallery .TravellersArchive-sync-slider-wrapper").appendChild(syncSliderClone);

			
			
			
			slider = new TravellersArchiveSlider('.TravellersArchive-slider-wrapper', { 
				...
				on: {	
					init : function(slide) {
						...
						if ($('body').hasClass("show-loader")) {
							...
							imagesLoaded('body', function() {
								...
								gsap.to($(".showcase-gallery .TravellersArchive-slide .slide-effects"), { duration: 0.5, opacity: 1, delay: 0.7, ease: Power4.easeIn, onComplete: function() {
									$(".showcase-gallery").addClass("active");
								}});
					...
					slideLeaveViewport : function( slide ) {
						gsap.set($('.TravellersArchive-slider div:not(.TravellersArchive-slide-visible) .slide-effects'), { x: "" });		
					},
				},
			});			
			
			const syncSliderCloneTranslate = document.querySelectorAll('.TravellersArchive-slider-wrapper.showcase-gallery .TravellersArchive-sync-slider-wrapper > .TravellersArchive-sync-slider-viewport');
			const titleWrapper = document.querySelector('.TravellersArchive-sync-slider-wrapper');
			
			slider.tl
				.fromTo('.TravellersArchive-slider-wrapper.showcase-gallery .TravellersArchive-sync-slider-wrapper', {yPercent: 0 },{ yPercent: -(100 - (100 / syncSliderCloneTranslate.length))},0)
				.fromTo('.hover-reveal', {y: 0}, {y: + (titleWrapper.offsetHeight/2)}, 0)
    

			//Slider Hover Events
			
			$('.TravellersArchive-sync-slide').on('mouseenter', function() {
				$('.TravellersArchive-sync-slide').addClass('disable'); 
				$(this).removeClass('disable').addClass('active'); 
			}).on('mouseleave', function() {
				$('.TravellersArchive-sync-slide').removeClass('disable active'); 
			});
			...
			Array.from(document.querySelectorAll('.TravellersArchive-sync-slide')).forEach(link => new HoverImgFx(link));
			...
			let gridPreview = new TravellersArchiveGridPreview( slider, {
			...
			gridPreview.on.beforeShowGrid = () => {
				...
				gsap.to($(".TravellersArchive-slide.hovered .slide-caption span"), {duration: 0.15, y:-30, opacity:0, stagger:0.05, ease:Power2.easeIn});
				gsap.to($(".TravellersArchive-slide .slide-caption"), {duration: 0.2, opacity:0, delay:0, ease:Power2.easeIn});
				gsap.to($(".TravellersArchive-slide .slide-thumb img"), {duration: 0.3, scale:0, opacity:0, ease:Power2.easeIn, onComplete: function() {
					gsap.set(".TravellersArchive-slide .slide-thumb", { scale: 0, opacity:0 });
				}});
			...
			gridPreview.on.flipShowGrid = (flipstateAllImages, flipstateCurrentImage) => {
				...
				gsap.to(".TravellersArchive-slider .TravellersArchive-slide .trigger-item", { duration: 1, opacity:0, scale:0.7, ease:Power2.easeOut });
				...
				if (document.getElementById('TravellersArchive-page-content').classList.contains('dark-content')) {
					if (gridPreview.currentImage.classList.contains('change-header')) {
						gsap.to('#TravellersArchive-logo img.black-logo', { duration: 0.5, delay:0.15, opacity: 0, ease:Power2.easeInOut });	
						gsap.to('#TravellersArchive-logo img.white-logo', { duration: 0.5, delay:0.15, opacity: 1, ease:Power2.easeInOut });						
				...
				} else if (document.getElementById('TravellersArchive-page-content').classList.contains('light-content')) {
					if (gridPreview.currentImage.classList.contains('change-header')) {
						gsap.to('#TravellersArchive-logo img.black-logo', { duration: 0.5, delay:0.15, opacity: 1, ease:Power2.easeInOut });	
						gsap.to('#TravellersArchive-logo img.white-logo', { duration: 0.5, delay:0.15, opacity: 0, ease:Power2.easeInOut });						
			...
			gridPreview.on.beforeHideGrid = () => {
				...
				gsap.to(".TravellersArchive-slider .TravellersArchive-slide .trigger-item", { duration: 0.5, opacity:1, scale:1, delay:0.2, ease: 'power3.inOut' });
				...
				gsap.set($(".TravellersArchive-slide.hovered .slide-caption span"), {y: 30 });				
				gsap.to($(".TravellersArchive-slide.hovered .slide-caption span"), {
					duration: 0.5,
					y: 0,
					opacity: 1,
					delay: 0.5,
					...
					ease: Power2.easeOut,
					onComplete: function() {
						gsap.set($(".TravellersArchive-slide .slide-caption span"), { clearProps: "all" });
					}
				});
				gsap.to($(".TravellersArchive-slide .slide-caption"), {duration: 0.3, opacity:1, delay:0.5, ease:Power2.easeIn});
				gsap.set(".TravellersArchive-slide .slide-thumb img", { scale: 1, opacity:1, onComplete: function() {
					gsap.to($(".TravellersArchive-slide .slide-thumb"), {duration: 0.5, scale: 1, delay:0.5, opacity:1, ease: "back.out(2)" });
				}});
			...
			gridPreview.on.flipHideGrid = (flipstateAllImages, flipstateCurrentImage) => {
				...
				Flip.from(flipstateCurrentImage, {
					...
					onComplete: function() {
						...
						const triggeredItem = document.querySelector('.TravellersArchive-slide.triggered-item');
						if( triggeredItem != null ){
							triggeredItem.classList.remove('triggered-item');
						}
						
						const TravellersArchiveSlides = document.querySelectorAll('.TravellersArchive-slide');
						TravellersArchiveSlides.forEach(slide => {
						  	slide.style.zIndex = '';
				...
				gsap.to('.header-gradient', { duration: 0.4, backgroundColor: $("#TravellersArchive-page-content").data("bgcolor"), ease:Power2.easeInOut });
				gsap.to("main", { duration: 0.4, backgroundColor: $("#TravellersArchive-page-content").data("bgcolor"), ease: Power2.easeInOut });
				
				
				if (document.getElementById('TravellersArchive-page-content').classList.contains('dark-content')) {
					if (gridPreview.currentImage.classList.contains('change-header')) {
						gsap.to('#TravellersArchive-logo img.black-logo', { duration: 0.4, opacity: 1, ease:Power2.easeInOut });	
						gsap.to('#TravellersArchive-logo img.white-logo', { duration: 0.4, opacity: 0, ease:Power2.easeInOut });						
					...
					if (gridPreview.currentImage.classList.contains('change-header')) {						
						gsap.to('#TravellersArchive-logo img.black-logo', { duration: 0.4, opacity: 0, ease:Power2.easeInOut });	
						gsap.to('#TravellersArchive-logo img.white-logo', { duration: 0.4, opacity: 1, ease:Power2.easeInOut });						
				...
				gsap.to($("footer .link-text, .TravellersArchive-pagination, .progress-info, #filters-wrapper"), {duration: 0.3, opacity:1, y:0, stagger:0.05, delay:0.4, ease:Power2.easeInOut});
				gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
				gsap.to('#ball-loader', {duration: 0.2, borderWidth: '4px', top: 0, left: 0});
				$("#ball").removeClass("with-blur");
				$('#ball p').remove();
			...
			if (!isMobile()) {
				...
				$('.TravellersArchive-slide').on('mouseenter', function() {
					$('.TravellersArchive-slide').removeClass('hovered');
					$(this).addClass('hovered');
				});
					
				
				$(".showcase-gallery.preview-mode-enabled .TravellersArchive-slide .slide-inner-height").on('mouseenter', function() {	
					if (!$('body').hasClass('scale-drag-y')) {
						$('#ball p').remove();
						var $this = $(this);			
						gsap.to('#ball', {duration: 0.3, borderWidth: '2px', scale: 1.4, borderColor:"rgba(170,170,170,0)", backgroundColor:"rgba(170,170,170,0.3)"});
				...
				$(".showcase-gallery .TravellersArchive-sync-slide .trigger-item").on('mouseenter', function() {	
					if (!$('body').hasClass('scale-drag-y')) {
						$('#ball p').remove();
						var $this = $(this);			
						gsap.to('#ball', {duration: 0.3, borderWidth: '2px', scale: 1.4, borderColor:"rgba(170,170,170,0)", backgroundColor:"rgba(170,170,170,0.3)"});
			...
			$('.grid-option').on('click', function () {
				...
				if (!$this.hasClass('active')) {
					...
					$(".TravellersArchive-sync-slider").removeClass("active");
			
					gsap.to($(".showcase-gallery .TravellersArchive-sync-slide .slide-title span"), {
						duration: 0.5,
						yPercent: -100,
						opacity: 0,
						stagger: 0.01,
					...
					gsap.to($(".showcase-gallery .TravellersArchive-slide .slide-effects"), {
						duration: 0.5,
						opacity: 1,
						delay: 0.5,
						ease: Power4.easeIn,
			...
			$('.list-option').on('click', function () {
				...
				if (!$this.hasClass('active')) {
					...
					gsap.set($(".TravellersArchive-sync-slide .slide-title span"), { yPercent: 100, opacity: 0 });
			
					gsap.to($(".TravellersArchive-sync-slide .slide-title span"), {
						duration: 0.7,
						yPercent: 0,
						opacity: 1,
						stagger: 0.02,
					...
					setTimeout(function () {
						$(".TravellersArchive-sync-slider").addClass("active");
					}, 1200);
				}
			});
			...
			$('.trigger-item').on('click', function() {
				...
				gsap.to($(".showcase-gallery .TravellersArchive-sync-slide .slide-title span"), {duration: 0.3, y: -70, opacity:0, ease:Power2.easeIn});
...
---------------------------------------------------*/
		...
		if( $('.snap-slider-holder').length > 0 ){
			...
			if (!$("body").hasClass("disable-ajaxload")) {
				$('.snap-slider-thumbs-wrapper .thumb-slide').on('click', function() {
					...
					if (!$("#TravellersArchive-page-content").hasClass("light-content")) {
						setTimeout(function() {
	...
	// ✅ Re-apply language after AJAX page load
	if (window.TravellersArchiveI18n) {
		window.TravellersArchiveI18n.applyLanguage(
